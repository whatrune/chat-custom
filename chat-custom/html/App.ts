...
export default Vue.extend({
  ...
  mounted() {
    post(`http://${resourceName}/loaded`, JSON.stringify({}));
  ...
  methods: {
    ...
    resetTabComplete() {
      this.tabCompleteBase = "";
      this.tabCompleteMatches = [];
      this.tabCompleteIndex = -1;
    },
    getCommandCompletionMatches(base: string): string[] {
      if (!base || !base.startsWith('/')) return [];
      // Only complete command name (first token)
      if (base.trim().includes(' ')) return [];

      return this.suggestions
        .map((s: Suggestion) => s.name)
        .filter((name: string, idx: number, arr: string[]) => arr.indexOf(name) === idx)
        .filter((name: string) => name.startsWith(base))
        .sort((a: string, b: string) => a.localeCompare(b));
    },
    keyDown(e: KeyboardEvent) {
      const inputEl = this.$refs.input as HTMLInputElement | undefined;
      ...
      if (e.which === 38 || e.which === 40) {
        ...
        this.resetTabComplete();
        var buf = document.getElementsByClassName("chat-messages")[0] as HTMLElement;
        ...
        // Chat command completion (only while chat input is focused and typing a slash command)
        if (isInputFocused && this.showInput && !e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
          const current = (this.message || '').trim();
          const canComplete = current.startsWith('/') && current.length > 1 && !current.includes(' ');

          if (canComplete) {
            let matches: string[] = [];
            let base = current;

            if (this.tabCompleteMatches.length > 0 && this.tabCompleteBase) {
              const currentNorm = current.replace(/\s+$/, '');
              const currentIsCycleValue = this.tabCompleteMatches.some((m: string) => `${m}` === currentNorm);
              if (currentIsCycleValue || currentNorm === this.tabCompleteBase) {
                matches = this.tabCompleteMatches;
                base = this.tabCompleteBase;
              }
            }

            if (matches.length === 0) {
              matches = this.getCommandCompletionMatches(base);
              this.tabCompleteBase = base;
              this.tabCompleteMatches = matches;
              this.tabCompleteIndex = -1;
            }

            if (matches.length > 0) {
              e.preventDefault();
              this.tabCompleteIndex = (this.tabCompleteIndex + 1) % matches.length;
              this.message = `${matches[this.tabCompleteIndex]} `;
        ...
        // Preserve original mode switching behavior for Shift+Tab / Alt+Tab
        if (e.shiftKey || e.altKey) {
          ...
          const buf = document.getElementsByClassName('chat-messages')[0] as HTMLElement;
          setTimeout(() => buf.scrollTop = buf.scrollHeight, 0);
        } else {
          // Plain Tab without a valid slash-command completion should do nothing special.
          // Important for servers that bind inventory to Tab.
          this.resetTabComplete();
          return;
        }
      } else {
    ...
    hideInput(canceled = false) {
      const inputNow = this.$refs.input as HTMLInputElement | undefined;
      if (inputNow) inputNow.blur();
      ...
      if (canceled) {
        post(`http://${resourceName}/chatResult`, JSON.stringify({ canceled }));
      }
      this.message = "";
